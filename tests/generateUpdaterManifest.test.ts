...
test('buildManifest produces a Tauri updater manifest with all platform entries', async () => {
  ...
  await writeFile(join(distDir, 'the-pair-1.1.11.app.tar.gz.sig'), 'mac-sig')
  await writeFile(join(distDir, 'the-pair-1.1.11-setup.nsis.zip.sig'), 'win-sig')
  await writeFile(join(distDir, 'the-pair-1.1.11.AppImage.tar.gz.sig'), 'linux-sig')
  const notesPath = join(root, 'release-notes.md')
  await writeFile(notesPath, 'Release notes go here')

  const manifest = buildManifest({
    version: '1.1.11',
    baseUrl: 'https://github.com/example/the-pair/releases/download/v1.1.11',
    distDir,
    notesPath,
  ...
  assert.equal(
    ...
    'https://github.com/example/the-pair/releases/download/v1.1.11/the-pair-1.1.11.app.tar.gz'
  )
